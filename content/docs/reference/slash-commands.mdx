...
<Callout type="info" title="The ones worth memorizing">
  `/plan` before big changes, `/debug` for an unknown defect, `/goal` for a durable multi-turn objective, `/fork` when a conversation should branch, and `/export` to share a session.
</Callout>
