...
<Callout type="info" title="Spaces (preview on main)">
  Synara `main` adds project spaces with `mod+alt+←` / `mod+alt+→` to switch spaces and `mod+alt+1`…`9` to jump. See [Features on main](/docs/reference/main-preview).
</Callout>

## Custom keybindings

A settings panel lets you explore and remap shortcuts.

<DocsScreenshot
  src="/docs-media/settings-shortcuts-light.png"
  alt="Keyboard shortcuts settings page showing the default keymap with remapping controls"
  width={1440}
  height={900}
  provenance="real"
  caption="The Shortcuts settings page lists the default keymap and lets you remap individual bindings."
/>

For full control, Synara reads user overrides from:
