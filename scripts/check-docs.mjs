...
export function parseFrontmatter(source) {
  ...
  for (const line of raw.split("\n")) {
    ...
    values[key] = value.replace(/^(?:"(.*)"|'(.*)')$/, (_, doubleQuoted, singleQuoted) =>
...
function routeForDocFile(file) {
  let relative = path.relative(DOCS_ROOT, file).replaceAll(path.sep, "/").replace(/\.mdx$/, "");
...
function validateMetaFile(file) {
  ...
  for (const entry of parsed.pages) {
    ...
    if (!existsSync(mdx) && !existsSync(childDirectory)) {
      errors.push(`${path.relative(ROOT, file)}: pages entry "${entry}" has no matching MDX file or directory`);
    }
  }
  return errors;
}
...
function validateGettingStartedContract() {
  ...
  for (const page of REQUIRED_GETTING_STARTED_PAGES) {
    if (!existsSync(path.join(directory, `${page}.mdx`))) {
      errors.push(`content/docs/getting-started/${page}.mdx: required Getting Started page is missing`);
    }
  }
  return errors;
}
...
export function collectDocumentationErrors() {
  ...
  for (const file of docs) {
    ...
    for (const field of ["title", "description"]) {
      if (typeof frontmatter.values[field] !== "string" || frontmatter.values[field].trim() === "") {
