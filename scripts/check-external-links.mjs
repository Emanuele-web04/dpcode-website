...
const ALLOWLIST = new Map([
  ...
  ["https://x.ai/cli/install.sh", "x.ai CLI install script (macOS/Linux); x.ai occasionally rate-limits bot agents"],
  ["https://x.ai/cli/install.ps1", "x.ai CLI install script (Windows); x.ai occasionally rate-limits bot agents"],
...
const rows = urls.map((url) => {
  ...
  return { url, verdict, status: String(r.status), finalUrl: r.finalUrl, error: r.error || "", sources: [...urlMap.get(url)].join(", ") };
});
...
if (warn.length > 0) {
  ...
  for (const r of rows.filter((x) => x.verdict === "WARN")) console.log(`  - ${r.url} -> ${r.status}`);
}
if (allowed.length > 0) {
