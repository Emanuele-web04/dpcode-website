...
test("documentation uses controlled media components instead of raw embeds", () => {
  for (const file of walk(CONTENT).filter((candidate) => candidate.endsWith(".mdx"))) {
    ...
    assert.doesNotMatch(source, /!\[[^\]]*\]\([^)]*\)/, `${relative} uses raw Markdown image syntax`);
    ...
    for (const component of ["DocsImage", "DocsScreenshot"]) {
      for (const block of componentBlocks(source, component)) {
        ...
        assert.match(block, /\bprovenance="(?:real|derived|diagram)"/, `${relative} ${component} needs provenance`);

        for (const match of block.matchAll(/\b(?:src|lightSrc|darkSrc)="([^"]+)"/g)) {
          const asset = match[1];
          assert.ok(asset.startsWith("/") && !asset.startsWith("//"), `${relative} media must be local`);
          ...
          if (asset.endsWith(".svg")) {
            ...
            assert.doesNotMatch(svg, /<script\b|<foreignObject\b|\son[a-z]+=|(?:href|src)=["']https?:/i, `${relative} references unsafe SVG ${asset}`);
          }
        }
      }
    }
    ...
    for (const block of componentBlocks(source, "DocsVideo")) {
      for (const required of ["src", "poster", "captions", "title", "transcript", "width", "height"]) {
        assert.match(block, new RegExp(`\\b${required}=`), `${relative} DocsVideo needs ${required}`);
      }
    }
  }
});
