...
function violations(result) {
  ...
  for (const [metric, maximum] of Object.entries(maximums)) {
    const actual = metric === "totalEncodedBytes"
  ...
  if (result.failedRequests.length !== 0) failures.push(`failedRequests: ${result.failedRequests.length}`);
  if (result.httpErrors.length !== 0) failures.push(`httpErrors: ${result.httpErrors.length}`);
  if (result.consoleErrors.length !== 0) failures.push(`consoleErrors: ${result.consoleErrors.length}`);
