...
test.describe("visual regression @visual", () => {
  ...
  test("homepage-workflows-desktop-light", async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 1100 });
    await preparePage(page, "light");
    const workflow = page.locator("section").filter({ hasText: "From objective to evidence" }).first();
  ...
  test("homepage-trust-mobile-light", async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await preparePage(page, "light");
    const trust = page.locator("section").filter({ hasText: "Know where every part of the work goes" }).first();
